...
\* \* При передаче очередного бита вычисляется значение некоторой функции, зависящей от значения очередного бита исходной последовательности (Аi) и значений уже переданных бит (Bi-1, Bi-2,…), например Bi = Ai ^ Bi-1 ^ Bi-2
